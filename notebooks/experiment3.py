...
with mlflow.start_run(run_name="XGBoost_ReturnRate"):
    ...
    perm = permutation_importance(xgb, X_test, y_test, n_repeats=5, random_state=42, n_jobs=-1)
    pi = pd.Series(perm.importances_mean, index=X_test.columns).sort_values(ascending=False)
    pi.to_csv("xgb_permutation_importance.csv")

    mlflow.log_artifact("xgb_permutation_importance.csv")
